...
describe('Reducers', () => {
  ...
  describe('profile', () => {
    it('saves the profile after loading', () => {
      ...
      expect(
        Reducers.profile(undefined, { type: AT.MARK_LOAD_PROFILE_SUCCESS, profile })
    ...
    it('clears the profile', () => {
      const profile = { 'foaf:img': 'https://example.com/img.jpg' }
      expect(
        Reducers.profile(profile, { type: AT.MARK_CLEAR_PROFILE })
  ...
  describe('endpoints', () => {
    ...
    it('clears endpoints', () => {
      expect(
        Reducers.endpoints(endpoints, { type: AT.MARK_CLEAR_ENDPOINTS })
      ).to.eql({
        login: null,
        logout: null,
        proxy: null,
  ...
  describe('bookmarks', () => {
    ...
    it('sets the bookmarks after a successful load', () => {
      ...
      const action = { type: AT.MARK_LOAD_SUCCESS, bookmarks: actionBookmarks }
      expect(Reducers.bookmarks(undefined, action)).to.eql(expectedBookmarks)
    })

    it('adds a bookmark after a successful save', () => {
      const bookmark = makeBookmark()
      const action = { type: AT.MARK_SAVE_BOOKMARK_SUCCESS, bookmark }
    ...
    it('replaces a bookmark after a successful save', () => {
      ...
      const action = { type: AT.MARK_SAVE_BOOKMARK_SUCCESS, bookmark: newBookmark }
    ...
    it('marks bookmarks as edited', () => {
      const bookmark = makeBookmark()
      const action = { type: AT.MARK_EDIT_BOOKMARK, bookmark }
      const state = Immutable.fromJS({
        [bookmark.get('@id')]: {
          data: bookmark,
          isEditing: false,
    ...
    it('removes a new/unsaved bookmark when editing is cancelled', () => {
      ...
      const action = { type: AT.MARK_EDIT_BOOKMARK_CANCEL, bookmark: tempBookmark }
      const state = Immutable.fromJS({
        [tempBookmark.get('@id')]: {
          data: tempBookmark,
          isEditing: true,
    ...
    it('cancels a saved bookmark from being edited', () => {
      const bookmark = makeBookmark()
      const action = { type: AT.MARK_EDIT_BOOKMARK_CANCEL, bookmark }
      const state = Immutable.fromJS({
        [bookmark.get('@id')]: {
          data: bookmark,
          isEditing: true,
    ...
    it('adds a new bookmark to the set', () => {
      const bookmark = makeBookmark()
      const action = { type: AT.MARK_CREATE_NEW_BOOKMARK, bookmark }
  ...
  describe('selectedTags', () => {
    it('adds tags to the selected set', () => {
      const action = { type: AT.MARK_FILTER_ADD_TAG, tag: 'a tag' }
      expect(
    ...
    it('removes tags from the selected set', () => {
      const action = { type: AT.MARK_FILTER_REMOVE_TAG, tag: 'a tag' }
      const state = Immutable.Set(['a tag'])
      expect(
  ...
  describe('showArchived', () => {
    it('flips the showArchived boolean to true or false', () => {
      const trueAction = { type: AT.MARK_FILTER_TOGGLE_ARCHIVED, shown: true }
      const falseAction = { type: AT.MARK_FILTER_TOGGLE_ARCHIVED, shown: false }
  ...
  describe('alerts', () => {
    ...
    it('clears the alert', () => {
      const action = { type: AT.MARK_ALERT_CLEAR, kind: 'danger' }
