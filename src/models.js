...
const vocab = {
  acl: term => rdflib.namedNode(`http://www.w3.org/ns/auth/acl#${term}`),
...
export function bookmarkModelFactory (graph, namedGraph, subject) {
  ...
  })(graph, namedGraph, subject).setAny(
    'type', 'http://www.w3.org/2002/01/bookmark#Bookmark', {namedNode: true}
  )
}

export function authorizationModelFactory (graph, namedGraph, subject) {
  return modelFactory(rdflib, {
    type: vocab.rdf('type'),
    agent: vocab.acl('agent'),
    accessTo: vocab.acl('accessTo'),
    mode: vocab.acl('mode')
  })(graph, namedGraph, subject).setAny(
    'type', 'http://www.w3.org/ns/auth/acl#Authorization', {namedNode: true}
  )
}
